  ...
  initiateEvents() {
    this.db.collection('quotes').valueChanges().subscribe((data: Quote[]) => {
      ...
      this.quotes.forEach((quote) => {
        console.log('quering for ',this.currentUser.uid, quote.id, 'in user liked quotes ');
        this.db.collection('userLikedQuotes', ref => ref.where('id', '==', this.currentUser.uid).
          where('likedQuotes', 'array-contains', quote.id)).get().subscribe(docs => {
            if (docs.size >= 1) {
              console.log(this.currentUser.uid, quote.id, 'in user liked quotes ');
              docs.forEach(doc => {
      ...
      this.quotes.forEach((quote) => {
        ...
        console.log('querying for ', quote.uid, 'in user details');
        // console.log('raw quote', JSON.stringify(quote));
        this.db.collection('users').doc(quote.uid).get().subscribe(docs => {
            if (docs.exists) {
              //console.log(this.currentUser.uid, quote.id);
              const userData = docs.data();
              quote.author = userData.displayName;
              quote.photoURL = userData.photoURL;
              quote.doNotShow = false;
              //console.log('final quote', JSON.stringify(quote))
            } else {
              quote.doNotShow = true;
            }
          });
      });
